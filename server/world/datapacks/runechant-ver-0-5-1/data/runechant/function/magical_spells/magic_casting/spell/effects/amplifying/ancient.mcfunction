...
execute at @e[type=minecraft:interaction,tag=AffinitySpellCheck] run tag @p add AncientEnergy
effect give @s minecraft:absorption 30 1
effect give @s minecraft:strength 30 1

execute unless entity @s[tag=amplifying_triple] unless entity @s[tag=amplifying_double] run tag @s remove amplifying_ancient
