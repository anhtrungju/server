...
execute at @e[type=minecraft:interaction,tag=AffinitySpellCheck] run tag @p add EnderEnergy
effect give @s minecraft:absorption 30 1
effect give @s minecraft:speed 30 1

execute unless entity @s[tag=amplifying_triple] unless entity @s[tag=amplifying_double] run tag @s remove amplifying_ender
