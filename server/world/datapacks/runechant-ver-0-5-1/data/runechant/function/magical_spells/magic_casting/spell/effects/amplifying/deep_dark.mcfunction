...
execute at @e[type=minecraft:interaction,tag=AffinitySpellCheck] run tag @p add DeepDarkEnergy
effect give @s minecraft:absorption 30 2
effect give @s minecraft:resistance 30 0

execute unless entity @s[tag=amplifying_triple] unless entity @s[tag=amplifying_double] run tag @s remove amplifying_deep_dark
