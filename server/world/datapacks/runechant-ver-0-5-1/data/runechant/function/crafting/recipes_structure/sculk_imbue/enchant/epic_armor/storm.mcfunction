...
execute unless data entity @s {Item:{components:{"minecraft:lore":['{"bold":false,"color":"light_purple","italic":false,"text":"Storm II (Electric Presence)"}']}}} run data modify entity @s Item.components."minecraft:lore" append value '{"bold":false,"color":"light_purple","italic":false,"text":"Storm II (Electric Presence)"}'
