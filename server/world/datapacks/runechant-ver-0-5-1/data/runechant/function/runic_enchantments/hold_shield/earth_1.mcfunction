...
execute if score *Timer3s Runechant_Timers matches 1 positioned ^ ^ ^2 at @e[type=#runechant:all,distance=..1.99,limit=1] run setblock ~ ~2 ~ minecraft:pointed_dripstone[vertical_direction=down] keep

execute if score *TimerTick Runechant_Timers matches 1 positioned ^ ^ ^2 as @e[type=#runechant:all,distance=..1.99,limit=1] run damage @s 1 minecraft:stalagmite